// Part 1 (Already Created 2D Array in Previous Lab)
...
data = "ID,Name,Occupation,Age\n42,Bruce,Knight,41\n57,Bob,Fry Cook,19\n63,Blaine,Quiz Master,58\n98,Bill,Doctor’s Assistant,26";

let odArray = data.split('\n');

let tdArray = []
for (let i of odArray){

    tdArray.push(i.split(','))
}

let keys = tdArray.splice(0, 1);
keys = keys[0];
...
for (let i = 0; i < tdArray.length; i++){
    ...
    for (let j = 0; j < keys.length; j++){
        part3Result[i][keys[j]] = tdArray[i][j]
    }
}
...
let sumAge = 0;
for (let i of part3Result){
    sumAge += Number(i.age)
}
let avgAge = sumAge/part3Result.length;
...
let part5Result = Object.keys(part3Result[0]);
part5Result = part5Result.join()

for (i of part3Result){
    // Old method
    // part5Result.push('\n')
    // part5Result = part5Result.concat(Object.values(i))
    // part5Result.push(...Object.values(i))
    part5Result = part5Result + '\n'
    part5Result = part5Result + Object.values(i).join()
}

// part5Result = part5Result.join()
console.log(part5Result)
